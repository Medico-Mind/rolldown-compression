...
`sectionSize` is the target number of bytes in each section when a large input is split across the native worker pool; inputs at least twice `sectionSize` take the multithreaded path. It defaults to two windows (`2^(windowBits + 1)` bytes) — 8 MiB and multithreading from 16 MiB at the default window — because sections much smaller than the window lose too many cross-section matches. The encoder segments between 64 KiB and 16 MiB, and values outside that range are clamped to it.
