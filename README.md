...
|------------------------|-------------------------------------------------|-------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `include`              | `string \| RegExp \| Array<string \| RegExp>`   | `/\.(html\|xml\|css\|json\|js\|mjs\|svg\|yaml\|yml\|toml\|txt\|wasm)$/` | Files to compress. Strings are [`path.matchesGlob`](https://nodejs.org/api/path.html#pathmatchesglobpath-pattern) patterns matched against bundle-relative file names. Source maps (`*.map`) are **not** covered by the default — add `/\.map$/` if you serve them pre-compressed.                                                                                                                                                                                                                                                                                                   |
...
| `filename`             | `string \| (fileName, algorithm) => string`     | `'[path][base]' + ext`                                                  | Name of the emitted artifact. Tokens: `[path]` (directory incl. trailing `/`), `[base]`, `[name]`, `[ext]` (with dot), `[hash]` (8-char sha256 of the **original**, uncompressed bytes). Default extensions: `.gz`, `.br`, `.zst`. The result must be a relative path inside the output directory, must differ from the source name, and must be unique across the build — anything else fails the build.                                                                                                                                                                                                                                                              |
| `deleteOriginalAssets` | `boolean`                                       | `false`                                                                 | Remove the original once **every** algorithm produced a variant of it. Files with no variant (e.g. skipped by `skipIfLargerOrEqual`) are always kept, so this can never leave the build without a servable copy. Removing chunks logs a warning — see [deleting originals](#deleting-originals).                                                                                                                                                                                                                                                                                                                                               |
...
| `logLevel`             | `'silent' \| 'error' \| 'warn' \| 'info'`       | `'info'`                                                                | Plugin log verbosity. `info` prints the per-algorithm summary through `this.info` and per-file detail through `this.debug`, so the bundler's own log level decides whether the detail is shown; `warn` keeps only warnings.                                                                                                                                                                                                                                                                                                                                                                                              |
...
`sectionSize` is the target number of bytes in each section when a large input is split across the native worker pool; inputs larger than one section are split. It defaults to two windows (`2^(windowBits + 1)` bytes) — 8 MiB at the default window — because sections much smaller than the window lose too many cross-section matches. The encoder segments between 64 KiB and 16 MiB, and values outside that range are clamped to it.

Smaller sections finish large files faster and cost ratio. Measured on 31.8 MiB of real JS at quality 9, `windowBits: 22`, against the 4 MiB point:

| `sectionSize` | sections in flight | output | vs 4 MiB |
|---------------|--------------------|--------|----------|
| 256 KiB       | 18                 | 6,123,466 | +14.9% |
| 1 MiB         | 18                 | 5,686,721 | +6.7%  |
| 2 MiB         | 16                 | 5,555,631 | +4.3%  |
| 4 MiB         | 8                  | 5,328,674 | —      |
| 8 MiB (default at `windowBits: 22`) | 4 | 5,244,683 | −1.6% |
| 16 MiB        | 2                  | 5,167,925 | −3.0%  |

**Brotli output is reproducible across machines.** How many sections an input is cut into follows `sectionSize` alone, and how many threads work through them is a scheduling decision that leaves the bytes alone, so a given input and set of options compress to identical bytes regardless of core count or [`concurrency`](#options) — as gzip and zstd already did.

**`windowBits` is the other speed/ratio dial.** It sets the brotli sliding window (`2^windowBits` bytes) — how far back a match can be found — and, through the `sectionSize` default above, how large a file has to be before it is split across threads. **Lowering it makes compression faster in two ways at once and costs compression ratio**: a smaller window is cheaper to search, *and* it reduces the default section size to `2^(windowBits + 1)` bytes, so big bundles are cut into sections sooner and finish on more cores (`windowBits: 20` splits inputs larger than 2 MiB instead of 8 MiB). Raising it does the reverse: better ratio, less parallelism, slower builds.
...
- Rolldown runs the output hooks once per output, so per-output state is reset in `renderStart` — one output's artifacts never shadow another output's sources, and watch-mode rebuilds don't accumulate state.
...
- The same filter, threshold and re-compression guards apply, and everything in the output directory that matches them is compressed — including files produced by other plugins after `generateBundle`, the default mode's limitation. Assets written in `closeBundle` (after all `writeBundle` hooks) are still not seen.
- `deleteOriginalAssets` is deliberately narrower than that: it only unlinks files the build actually wrote — those the bundle declares, or those written while the output was being generated. A leftover from an earlier build in a directory that was not emptied gets compressed but never deleted, because deleting a file the build did not produce is not something a compression plugin can undo.
...
`deleteOriginalAssets: true` removes an original only once a compressed variant of it exists. Anything the compressor skipped — incompressible binaries, files under a codec's break-even point — stays in place, and the plugin says which ones it kept. In [stream mode](#stream-mode) it additionally only touches files the build itself wrote.

Removing **chunks** (JS/CSS) is a deployment decision, not just a bundling one: only the compressed name is left on disk, so requests resolve only if whatever serves the build rewrites the original path to the compressed variant. Dynamic imports and source map links still use the original names. The plugin warns once per process the first time it removes a chunk.

### Plugin identity and `api`

The plugin follows the rolldown [plugin conventions](https://rolldown.rs/apis/plugin-api#conventions): it is named `rolldown-plugin-compression` and reports the package it ships in, so tooling that inspects a build can attribute it.

```ts
const plugin = compression()
plugin.name             // 'rolldown-plugin-compression'
plugin.version          // the package version
plugin.meta?.packageName // '@medicomind/rolldown-compression'
```

Other plugins can read its `api` through the documented [inter-plugin communication](https://rolldown.rs/apis/plugin-api/inter-plugin-communication) channel — useful for, say, a middleware that serves the artifacts:

```ts
interface CompressionPluginApi {
  packageName: string
  algorithms: readonly ('gzip' | 'brotli' | 'zstd')[]
  extensions: readonly string[]          // '.gz', '.br', '.zst'
  emittedFileNames(): string[]           // artifacts of the latest output
}
```
...
- **brotli at quality 11** is the bound on the combined number: the Rust encoder is at per-core parity with node's C brotli (we measured a 1.01 single-thread ratio), so the speedup is parallelism — every core against `UV_THREADPOOL_SIZE` threads across the many small files, plus the split worker pool on the >16 MiB bundles that `node:zlib` has to compress one thread per file.
> [!WARNING]
> **The brotli rows below predate the `mbrotli` migration and have not been re-measured.** That release replaced the encoder, and a section now costs more ratio to split than it used to. Expect the brotli numbers to have moved substantially in both wall clock and output size; the gzip and zstd rows are unaffected. Re-run `npm run bench` for current figures.

- The speedup grows with core count and shrinks if you raise `UV_THREADPOOL_SIZE` for the JS side — the benchmark prints both so runs are comparable. The 10-core M1 Pro these tables previously covered landed around 2.4x on the combined scenario against the same 4-thread JS side.
